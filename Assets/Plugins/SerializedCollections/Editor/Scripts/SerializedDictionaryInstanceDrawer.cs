...
namespace AYellowpaper.SerializedCollections.Editor
{
    ...
    {
        const float MinKeyValueLabelWidth = 40f;
        readonly SerializedDictionaryAttribute _dictionaryAttribute;

        readonly FieldInfo _fieldInfo;
        readonly FieldInfo _keyFieldInfo;
        readonly IReadOnlyList<KeyListGeneratorData> _keyGeneratorsWithoutWindow;
        readonly IReadOnlyList<KeyListGeneratorData> _keyGeneratorsWithWindow;
        readonly GUIStyle _keyValueStyle;
        readonly List<int> _pagedIndices;
        readonly PagingElement _pagingElement;
        readonly PropertyData _propertyData;
        readonly SearchField _searchField;
        readonly SingleEditingData _singleEditingData;
        readonly ReorderableList _unexpandedList;
        ListState _activeState;
        GUIContent _detailsContent;
        GUIContent _label;
        int _lastListSize = -1;
        bool _propertyListSettingsInitialized;
        GUIContent _shortDetailsContent;
        bool _showSearchBar;
        Rect _totalRect;
        ...
        void DoList(Rect position)
        {
            ...
            {
                using (new GUI.ClipScope(new Rect(0,
                       position.y,
        ...
        SerializedProperty GetElementProperty(SerializedProperty property, bool fieldFlag)
        {
        ...
        static Type FindGenericBaseType(Type generic, Type toCheck)
        {
        ...
        IKeyable GetLookupTable(object dictionary)
        {
        ...
        ReorderableList MakeUnexpandedList()
        {
        ...
        void ToggleSearchBar(bool flag)
        {
            _showSearchBar = flag;
        ...
        void OnDrawNoneElement(Rect rect)
        {
            EditorGUI.LabelField(rect, EditorGUIUtility.TrTextContent(_activeState.NoElementsText));
        }

        (DisplayType displayType, bool canToggleListDrawer) CreateDisplaySettings(SerializedProperty property,
            Type type)
        {
        ...
        void OnDrawUnexpandedHeader(Rect rect)
        {
        ...
        void DoOptionsButton(Rect rect)
        {
            ...
            if (GUI.Button(rect, EditorGUIUtility.IconContent("pane options@2x"), EditorStyles.iconButton))
            {
                ...
                SCEditorUtility.AddGenericMenuItem(gm,
                false,
                ...
                gm.AddItem(new GUIContent("Preferences..."),
                false,
        ...
        void OnPopulatorDataSelected(object userData)
        {
        ...
        void OpenKeysGeneratorSelectorWindow(Rect rect)
        {
        ...
        void ToggleAlwaysShowSearchPropertyData()
        {
            _propertyData.AlwaysShowSearch = !_propertyData.AlwaysShowSearch;
            SavePropertyData();
        }

        void DoKeyValueRect(Rect rect)
        {
        ...
        float GetDesiredKeyLabelWidth(float maxWidth, float offset = 0f)
        {
        ...
        void ApplyPopulatorQueued(KeyListGenerator populator, ModificationType modificationType)
        {
        ...
        void QueueAction(EditorApplication.CallbackFunction action)
        {
            EditorApplication.delayCall += action;
        }

        void ApplyPopulator(IEnumerable<object> elements, ModificationType modificationType)
        {
        ...
        static void AddElements(IKeyable lookupTable, IEnumerable<object> elements)
        {
        ...
        static void ConfineElements(IKeyable lookupTable, IEnumerable<object> elements)
        {
        ...
        static void RemoveElements(IKeyable lookupTable, IEnumerable<object> elements)
        {
            var indicesToRemove =
        ...
        void DoDisplayTypeToggle(Rect contentRect, bool fieldFlag)
        {
            ...
            if (displayData.Settings.HasListDrawerToggle)
            {
                ...
                EditorGUI.BeginChangeCheck();
                var newValue = GUI.Toggle(rightRectToggle,
        ...
        float OnGetElementHeight(int index)
        {
            ...
            _propertyData.GetElementData(SerializedDictionaryDrawer.KeyFlag).EffectiveDisplayType == DisplayType.List,
        ...
        void DrawGroupedElement(Rect rect, int spaceForProperty, SerializedProperty property, DisplayType displayType)
        {
        ...
        {
            var targetIndex = list.selectedIndices.Count > 0 && list.selectedIndices[0] >= 0
                ? list.selectedIndices[0]
                : 0;
            var actualTargetIndex = targetIndex < _pagedIndices.Count ? _pagedIndices[targetIndex] : 0;
