...
namespace Dreamteck.Splines
{
    ...
    {
        Texture2D basePreview;
        Texture2D brushPreview;
        public float clipFrom;
        public float clipTo = 1f;
        Texture2D drawPreview;
        ...
        public override void Draw(Rect windowRect)
        {
            ...
            GUILayout.BeginHorizontal();
            GUILayout.Box("",
            GUILayout.Width((windowRect.width - 10) / 2),
        ...
        void OnFocus()
        {
            ...
            {
                var isChanged = false;
                var newHeights = terrain.terrainData.GetHeights(0,
                0,
        ...
        void CarveTerrain()
        {
            ...
            for (var x = 0; x < drawLayer.GetLength(0); x++)
            {
                for (var y = 0; y < drawLayer.GetLength(1); y++)
                {
                    ...
                    pixels[x * drawPreview.width + y] = Color.Lerp(Color.black,
                    Color.white,
                    blurLayer[x, y] / maxDrawHeight * blurAlphaLayer[x, y]);
                }
            }
        ...
        void GetBase()
        {
            ...
            heights = terrain.terrainData.GetHeights(0,
            0,
            terrain.terrainData.heightmapResolution,
            terrain.terrainData.heightmapResolution);
        ...
        void PaintSegment(TerrainPaintPoint fromPoint, TerrainPaintPoint toPoint, ref float[,] layer,
                        ...
                        if (overWriteHeight || layer[current.x, current.y] == 0f)
                        {
                            if (!ContainsPoint(ref drawn, current))
                            {
                                ...
                                if (writeAlpha)
                                    ...
                                    current.y,
                                    height,
                                    alphaLayer[current.x, current.y],
                                    ref alphaLayer,
                                    ref layer);
                                drawn.Add(current);
                            }
