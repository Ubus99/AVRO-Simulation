            ...
            if (channelCount > 0)
            {
                ...
                if (morph.GetSnapshot(0).Length != morph.spline.pointCount)
                {
                    EditorGUILayout.HelpBox(
                        "Recorded morphs require the spline to have " + morph.GetSnapshot(0).Length +
                        ". The spline has " + morph.spline.pointCount, MessageType.Error);
                    ...
                    if (GUILayout.Button("Update Morph States"))
                        if (EditorUtility.DisplayDialog("Update morph states?",
                            ...
                            for (var i = 0; i < morph.GetChannelCount(); i++)
                            {
