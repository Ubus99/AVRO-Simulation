...
namespace Dreamteck.Splines
{
    ...
    public class SurfaceGenerator : MeshGenerator
    {
        ...
        public void Generate()
        {
            ...
            var surfaceVertexCount = sampleCount;
            if (spline.isClosed) surfaceVertexCount--;
            var vertexCount = surfaceVertexCount;
            ...
            var simpleExtrude = !pathExtrude && _extrude != 0f;
            ...
            var clockwise = IsClockwise(projectedVerts);
            var flipCap = false;
            var flipSide = false;
            ...
            var totalTrisCount = surfaceTris.Length;
            if (simpleExtrude)
            {
                totalTrisCount *= 2;
            ...
            var off = _trsRight * offset.x + _trsUp * offset.y + _trsForward * offset.z;
            ...
            var min = projectedVerts[0];
            var max = projectedVerts[0];
            for (var i = 1; i < projectedVerts.Length; i++)
            {
            ...
            if (pathExtrude)
            {
                ...
                for (var i = 0; i < surfaceVertexCount; i++)
                {
                    var vertexOffset = TransformOffset(extrudeResults[0], _extrudeOffset);
                ...
                //Add wall vertices
                var totalLength = 0f;
                for (var i = 0; i < extrudeResults.Length; i++)
                {
                    if (_uniformUvs && i > 0)
                        totalLength += Vector3.Distance(extrudeResults[i].position, extrudeResults[i - 1].position);
                    var startIndex = surfaceVertexCount * 2 + i * sampleCount;
                    for (var n = 0; n < identityVertices.Length; n++)
                    {
                        var vertexOffset = TransformOffset(extrudeResults[i], _extrudeOffset);
                ...
                var written = WriteTris(ref surfaceTris, ref _tsMesh.triangles, 0, 0, false);
                written = WriteTris(ref surfaceTris, ref _tsMesh.triangles, surfaceVertexCount, written, true);

                MeshUtility.GeneratePlaneTriangles(ref wallTris,
                sampleCount - 1,
                extrudeResults.Length,
                flipSide,
                0,
                0,
                true);
            ...
            else if (simpleExtrude)
            {
                ...
                for (var i = 0; i < surfaceVertexCount + 1; i++)
                {
                    ...
                    (float)i / (surfaceVertexCount - 1) * _sideUvScale.x + _sideUvOffset.x,
                    0f + _sideUvOffset.y);
                    if (clockwise)
                    {
                    ...
                    var offsetIndex = i + surfaceVertexCount * 3 + 1;
        ...
        {
            for (var i = trisOffset; i < trisOffset + tris.Length; i += 3)
            {
                if (flip)
                {
        ...
        bool IsClockwise(Vector2[] points2D)
        {
            ...
            {
                var v1 = points2D[i];
                var v2 = points2D[(i + 1) % points2D.Length];
        ...
        {
            var vertsRotation = Quaternion.Inverse(Quaternion.LookRotation(normal));
        ...
        void GetProjectedVertices(int count, out Vector3 center, out Vector3 normal)
        {
            ...
            {
                GetSampleRaw(i, ref evalResult);
                var point = evalResult.position + off - center;
                var projectionPointX = Vector3.Project(point, right).magnitude;
                if (Vector3.Dot(point, right) < 0.0f) projectionPointX *= -1f;
                var projectionPointY = Vector3.Project(point, up).magnitude;
