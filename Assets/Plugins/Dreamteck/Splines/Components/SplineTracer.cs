    ...
    public class SplineTracer : SplineUser
    {
        ...
        protected void ApplyMotion()
        {
            ...
            switch (_physicsMode)
            {
                case PhysicsMode.Transform:
                    if (!targetTransform) RefreshTargets();
                ...
                case PhysicsMode.Rigidbody:
                    ...
                    {
                        RefreshTargets();
                        if (!targetRigidbody)  throw new MissingComponentException("There is no Rigidbody attached to " + name + " but the Physics mode is set to use one.");
                    }
                ...
                case PhysicsMode.Rigidbody2D:
                    ...
                    {
                        RefreshTargets();
                        if (!targetRigidbody2D) throw new MissingComponentException("There is no Rigidbody2D attached to " + name + " but the Physics mode is set to use one.");
                    }
        ...
        protected void InvokeNodes()
        {
            if (nodeConnectionQueue.Count <= 0)
                return;
            onNode?.Invoke(nodeConnectionQueue);
